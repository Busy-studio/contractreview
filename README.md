...
- **GPT-5.6 Sol**: 근거 수준에 따라 핵심 검토조항을 분류하고 수정 문안 작성
- **선택적 판례 검색**: 법령·행정규칙·내부규정 근거가 약한 쟁점에 한해 Terra가 최대 3개 판례 검색어를 생성
- **근거 기반 분류**: 법적 위험 / 판례상·분쟁 위험 / 내부규정 검토 / 협상 권고
- **간소화된 결과**: 각 검토조항을 원문 → 문제되는 이유 → 확인된 근거 → 변경 예시 순서로 표시
...
주요 검토조항은 중요도 중심으로 최대 8개까지 제시하며, 서로 연결된 조항은 하나의 항목으로 묶습니다.

분류 기준:
- 법적 위험: 직접 적용 가능한 법령·행정규칙 근거가 실제 확인된 경우
- 판례상·분쟁 위험: 유사 쟁점 판례가 판단 근거를 제공하는 경우
- 내부규정 검토: 대학 내부규정·지침과 충돌하거나 별도 절차가 필요한 경우
- 협상 권고: 직접 법적 근거는 없지만 계약상 책임·비용·재량 배분이 불리하거나 불명확한 경우

협상 권고 항목에는 법적 근거를 억지로 만들지 않으며, 별도의 근거 섹션을 생략합니다.
...
화면, DOCX, PDF 모두 동일한 정보 위계를 사용합니다.
...
PDF는 실제 텍스트 레이어를 유지하여 검색·선택·복사가 가능합니다.
